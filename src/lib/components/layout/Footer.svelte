...
<footer style="background: var(--surface); border-top: 1px solid var(--border);">
  <div class="container mx-auto max-w-6xl px-6 py-12 flex flex-col md:flex-row justify-between items-center gap-6 text-center md:text-left">
    
    <div>
      <h4 class="font-bold text-lg" style="color:var(--text);">Boxgramer</h4>
      <p class="text-sm mt-1" style="color:var(--text-muted);">Gameplay Programmer & Software Engineer</p>
    </div>

    {#if hasNav}
      <nav class="flex flex-wrap justify-center gap-6">
        {#each navLinks as link}
          <a href={link.href} class="text-sm font-semibold transition-colors hover:text-[var(--accent)]" style="color:var(--text-muted);">
            {link.label}
          </a>
        {/each}
      </nav>
    ...
    <div class="flex gap-3">
      {#each socials as s}
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" viewBox={s.vb} fill="currentColor" style="width:16px;height:16px;">
            <path d={s.path}/>
          </svg>
        </a>
      {/each}
