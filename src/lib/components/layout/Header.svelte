<script>
  import logo from '$lib/assets/avatar.png'; // replace with your logo image
</script>

<header class="max-h-80 h-20 bg-white shadow-md flex justify-center items-center">
  <nav class="container mx-auto flex justify-between items-center">
    <ul class="flex justify-center items-center">
      <li class="mr-6">
        <a href="#" class="text-lg font-bold text-gray-600 hover:text-gray-900">Home</a>
      </li>
      <li class="mr-6">
        <a href="#" class="text-lg font-bold text-gray-600 hover:text-gray-900">Journey</a>
      </li>
      <li class="mx-6">
        <img src={logo} alt="Logo" class="h-12 w-12" />
      </li>
      <li class="mr-6">
        <a href="#" class="text-lg font-bold text-gray-600 hover:text-gray-900">Project</a>
      </li>
      <li>
        <a href="#" class="text-lg font-bold text-gray-600 hover:text-gray-900">Contact</a>
      </li>
    </ul>
  </nav>
</header>